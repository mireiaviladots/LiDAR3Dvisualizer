...
# Parámetros de IDW
radio = 0.5  # Radio de búsqueda (en las mismas unidades que las coordenadas UTM)
p = 2  # Exponente de ponderación

# Interpolación para cada punto del área
dosis_nube = np.full(len(puntos_dentro), np.nan)  # Vector para almacenar las dosis interpoladas

for i, punto in enumerate(puntos_dentro[:, :2]):  # Itera sobre cada punto x, y
    indices = tree.query_ball_point(punto, r=radio)  # Encuentra los puntos dentro del radio

    if len(indices) > 0:  # Solo si hay vecinos dentro del radio
        dosis_nube[i] = idw_interpolation(punto, utm_points[indices], dosis[indices], p=p)
    else:
        # Encontrar el vecino más cercano (fuera del radio)
        _, indice_cercano = tree.query(punto, k=1)
        dosis_nube[i] = dosis[indice_cercano]  # Asignar la dosis
...
output_file = Path("cloud0.5retorno_con_dosis_idwRadio.las")
